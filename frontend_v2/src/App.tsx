...
import { Loader2, Search, Trash2, History } from 'lucide-react';
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            {jobsData?.jobs.map(job => (
                                <div
                                    key={job.scan_id}
                                    className={`group flex items-center justify-between p-2 rounded-md text-sm cursor-pointer transition-colors ${scanId === job.scan_id ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-muted'}`}
                                    onClick={() => loadJob(job.scan_id)}
                                >
                                    <div className="truncate flex-1">
                                        <p className="truncate">{job.query || "Unknown Scan"}</p>
                                        <p className="text-[10px] text-muted-foreground">{new Date(job.start_time).toLocaleDateString()}</p>
                                    </div>
                                    <Button
                                        variant="ghost"
                                        size="icon"
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                ...
                <ScrollArea className="flex-1">
                    <div className="p-6 space-y-8 pb-20 max-w-[1600px] mx-auto">
                        ...
                        {scanId && !inventory.length && !results.length && activeScan?.stage !== 'complete' && (
